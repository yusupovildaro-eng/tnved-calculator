...
def expand_query(q: str):
    ...
    q_low = q.lower().strip()
    # Проверяем точное совпадение слова/фразы в SYNONYMS
    expanded = []
    for key, synonyms in SYNONYMS.items():
        if key in q_low:
            expanded.extend(synonyms)
    # Если нашли синонимы — используем их + оригинальные слова
    if expanded:
...
def smart_search(q: str, limit: int = 18):
    ...
    score_cases = '\n'.join(
        [f"+ CASE WHEN name_ru LIKE ? THEN 10 ELSE 0 END" for _ in terms]
    )
    where_clauses = ' OR '.join(['name_ru LIKE ?' for _ in terms])
